...
def _router_assist_mode(rt: dict[str, Any]) -> str:
    settings = _settings(rt)
    mode = (
        str(getattr(settings, "agent_graph_llm_router_assist_trigger_mode", "low_confidence") or "low_confidence")
        .strip()
        .lower()
    )
...
def _router_assist_should_trigger(
    *,
    ...
    if subject_domain == "generic" and any(
        tok in text for tok in ("账单", "保险", "家电", "宠物", "房贷", "网络", "宽带", "nbn")
    ):
    ...
    if any(tok in text for tok in ("提供商", "provider", "vendor", "运营商", "服务商")) and any(
        tok in text for tok in ("联系方式", "电话", "邮箱", "contact")
    ):
...
def _router_assist_apply_result(
    *,
    ...
    if not selected:
        ...
        reason_tags = [
            str(x or "").strip().lower() for x in (raw_result.get("reason_tags") or []) if str(x or "").strip()
        ]
        ...
        if llm_conf >= 0.55:
            if any("specific_sub_category" in tag or "billing" in tag for tag in reason_tags):
                if (
                    any(tok in " ".join(aliases) for tok in ("internet", "nbn", "superloop", "broadband"))
    ...
    if final_categories:
        ...
        if llm_conf >= 0.85 and str(out_spec.get("subject_domain") or "generic") == "generic":
            if any(str(c).startswith("finance/bills") for c in final_categories):
                ...
                str(c).startswith(("home/insurance", "health/insurance", "legal/insurance")) for c in final_categories
            ):
    ...
    diag = {
        ...
        "reason_tags": [str(x or "").strip() for x in (raw_result.get("reason_tags") or []) if str(x or "").strip()][
            :8
        ],
    }
...
def _repair_query_spec_with_rules(
    *,
    ...
    if str(repaired.get("task_kind") or "") == "list" and str(rule_spec.get("task_kind") or "") == "fact_lookup":
        if "bill_amount" in list(repaired.get("target_slots") or []) or "bill_amount" in list(
            rule_spec.get("target_slots") or []
        ):
...
def structured_fastpath_node(state: AgentGraphState, config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    stats_kwargs: dict[str, Any] = {
        ...
        "coverage_missing_fields": [
            str(x) for x in (bundle.get("coverage_missing_fields") or []) if str(x or "").strip()
        ],
        ...
        if str((card.short_summary.zh if card and card.short_summary else "") or "").strip()
    ...
    return {
        ...
        "executor_stats_payload": {
            **dict(state.get("executor_stats_payload") or {}),
            ...
            "retrieval_mode": "+".join(sorted(x for x in retrieval_modes if x and x != "none"))
...
def rerank_candidates_node(state: AgentGraphState, config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    preferred_categories = [
        str(x or "").strip().lower() for x in (spec.get("preferred_categories") or []) if str(x or "").strip()
    ]
...
def _graph_has_slot_values(slot_results: list[dict[str, Any]]) -> bool:
    return any(
        str(item.get("status") or "") in {"found", "derived"} and str(item.get("value") or "").strip()
...
def _graph_has_derived_values(derivations: list[dict[str, Any]]) -> bool:
    return any(
        str(item.get("status") or "") == "derived" and str(item.get("value") or "").strip()
...
def _build_graph_evidence_outputs(
    *,
    ...
    has_slot_doc_only = any(
        str(doc_id or "").strip() for item in slot_results for doc_id in list(item.get("source_doc_ids") or [])
    )
    if slot_sources:
        evidence_link_quality = "slot_evidence_first"
        related_doc_selection_mode = (
            "slot_evidence_first" if len(sources) <= len(slot_sources[:5]) else "slot_evidence_plus_context"
        )
...
def answer_build_node(state: AgentGraphState, config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    critical_missing = [str(x or "") for x in (state.get("critical_missing_slots") or []) if str(x or "").strip()]
    partial_evidence_signals = [
        str(x or "") for x in (state.get("partial_evidence_signals") or []) if str(x or "").strip()
    ]
    ...
    slot_evidence_doc_ids = [
        str(x or "") for x in (evidence_outputs.get("slot_evidence_doc_ids") or []) if str(x or "").strip()
    ]
    ...
    cov_stats = DetailCoverageStats(
        docs_scanned=max(
            len(
                {str(x.get("doc_id") or "") for x in (state.get("candidate_hits") or []) if str(x.get("doc_id") or "")}
            ),
            len({str(x.get("doc_id") or "") for x in context_chunks if str(x.get("doc_id") or "")}),
        ),
    ...
    bundle = {
        ...
        if str(query_spec.get("task_kind") or "") in {"fact_lookup", "status_check", "howto_lookup", "detail_extract"}
        else "search_bundle",
    ...
    answer_mode = (
        ...
        if answer_posture == "direct"
            ...
            else ("evidence_only_structured" if answer_posture == "evidence_only" else "refusal")
        )
    )
    ...
    if force_refusal:
        ...
        card.evidence_summary = [
            f"{slot}:{1 if slot not in missing_slots else 0}" for slot in list(state.get("required_slots") or [])[:8]
        ]
    ...
    executor_stats_payload = {
        ...
        else ("en_native" if str(card.short_summary.en or "").strip() else "bilingual_fallback"),
        "answer_mode": answer_mode,
