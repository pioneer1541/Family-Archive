...
class TestAgentV2Comparison:
    ...
    @pytest.mark.skip(reason="Phase 2: New graph structure with classifier - needs LLM mocking")
    def test_chitchat_comparison(self):
        ...
        # Note: Phase 2 uses unified synthesizer which requires LLM mocking
        ...
        # Handle both dict and Pydantic model for card
        card_type = getattr(new_result.card, "type", None)
        assert new_result.executor_stats.route == "chitchat" or card_type == "chitchat"
