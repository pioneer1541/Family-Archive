...
def _search_vector_query(query: str, category_path: str | None, score_threshold: float, top_k: int) -> list[dict[str, Any]]:
    try:
        hits = search_records(query=query, top_k=top_k, score_threshold=score_threshold, category_path=category_path)
    except Exception:
        return []
