...
def refresh_sync_run_status(db: Session, run_id: str) -> SyncRun | None:
    ...
    if source_paths:
        doc_rows = (
            db.execute(
                select(Document).where(Document.source_path.in_(source_paths)).order_by(Document.updated_at.desc())
            )
            .scalars()
            .all()
        )
    ...
    if is_active:
        ...
        run.finished_at = run.finished_at or dt.datetime.now(dt.UTC)
        run.status = (
            "failed" if int(counts.get("failed", 0)) > 0 and int(counts.get("completed", 0)) == 0 else "completed"
        )
    else:
        run.status = "running"
        run.finished_at = None
