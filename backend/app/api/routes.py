...
def list_gmail_credentials(db: Session = Depends(get_db)):
    ...
    creds = (
        db.execute(select(GmailCredentials).where(GmailCredentials.is_active is True))
        .scalars()
        .all()
    )
    items = []
    for c in creds:
        client_id_masked = (
            c.client_id[:8] + "..." + c.client_id[-4:]
            if len(c.client_id) > 12
            else c.client_id
        )
        items.append(
            {
...
def create_gmail_credentials(body: dict, db: Session = Depends(get_db)):
    ...
    cred = GmailCredentials(
        ...
        refresh_token_encrypted=encrypt(body["refresh_token"])
