...
def ensure_sqlite_runtime_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "mail_ingestion_events" in tables:
            mail_cols = {
                str(row[1] or "")
                for row in conn.execute(text("PRAGMA table_info(mail_ingestion_events)"))
            }
            if "sync_run_id" not in mail_cols:
                conn.execute(text("ALTER TABLE mail_ingestion_events ADD COLUMN sync_run_id VARCHAR(36)"))
            conn.execute(text("CREATE INDEX IF NOT EXISTS ix_mail_ingestion_events_sync_run_id ON mail_ingestion_events(sync_run_id)"))
