...
def test_register_success_password_hashed(admin_client):
    payload = {"username": "alice", "password": "StrongPass123!"}
    response = admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"})
...
def test_register_duplicate_username_fails(admin_client):
    payload = {"username": "bob", "password": "StrongPass123!"}
    assert admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"}).status_code == 201

    duplicate_resp = admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"})
    assert duplicate_resp.status_code == 422
    assert duplicate_resp.json() == {"detail": "Username already exists."}


def test_login_fails_with_invalid_credentials(admin_client):
    payload = {"username": "member", "password": "StrongPass123!"}
    assert admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"}).status_code == 201

    response = admin_client.post("/api/v1/auth/login", json={"username": payload["username"], "password": "wrong-password"})
    assert response.status_code == 401


def test_jwt_generation_and_verification(admin_client):
    payload = {"username": "jwtuser", "password": "StrongPass123!"}
    register = admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"})
...
def test_change_password_patch_works(admin_client):
    payload = {"username": "changepw", "password": "StrongPass123!"}
    assert admin_client.post("/api/v1/auth/users", json={**payload, "role": "user"}).status_code == 201
    login = admin_client.post("/api/v1/auth/login", json=payload)
    assert login.status_code == 200

    change = admin_client.put(
    ...
    relogin = admin_client.post(
        "/api/v1/auth/login",
        json={"username": payload["username"], "password": "NewStrongPass123!"},
    )
...
def test_non_admin_cannot_manage_users(admin_client):
    ...
    login = admin_client.post("/api/v1/auth/login", json=payload)
    assert login.status_code == 200
    user_cookie = _extract_cookie_value(login.headers.get("set-cookie", ""), COOKIE_NAME)
    assert user_cookie

    r1 = admin_client.get("/api/v1/auth/users", cookies={COOKIE_NAME: user_cookie})
    assert r1.status_code == 403

    r2 = admin_client.post(
        "/api/v1/auth/users",
        json={"username": "xuser", "password": "StrongPass123!"},
        cookies={COOKIE_NAME: user_cookie},
    )
    assert r2.status_code == 403


def test_error_response_format_for_validation_errors(admin_client):
    response = admin_client.post("/api/v1/auth/users", json={"username": "u1", "password": "123", "role": "user"})
...
def test_me_response_returns_username(admin_client):
